...
export default class RunUITests extends ReactNativeAppCommand {
	...
	public async run(): Promise<boolean | void> {
		...
		const isIOSplatform: boolean =
			this._app.os.toLowerCase() === AppCenterOS.iOS.toLowerCase();
		const platformDir = isIOSplatform
			? ReactNativePlatformDirectory.IOS
			: ReactNativePlatformDirectory.Android;
