...
export default abstract class AppCenterUITestRunner {
	...
	public constructor(protected options: TestRunnerOptions) {
		this.nativeAppDirectory = path.join(
			options.appDirPath,
			options.platformDir
		);
	}
	...
	public async runUITests(async: boolean): Promise<boolean> {
		return await VsCodeUI.showProgress<boolean>(async (progress) => {
			progress.report({
				message: Messages.CheckingAppCenterCliProgressMessage,
			});
			if (!(await Utils.packageInstalledGlobally("appcenter-cli"))) {
				VsCodeUI.ShowWarningMessage(
					Messages.PackageIsNotInstalledGloballyWarning(
						"appcenter-cli"
					)
				);
				return false;
			...
			progress.report({
				message: Messages.FetchingDevicesProgressMessage,
			});
			const devices: TestQuickPickItem[] = await this.getDevicesList(
				this.options.app
			);
			const deviceSets: TestQuickPickItem[] =
				await this.getDeviceSetsList(this.options.app);
			devices.unshift(...deviceSets);
			const selectedDevice: TestQuickPickItem =
			...
			if (selectedDevice.type === TestDeviceType.Device) {
				shortDeviceId = await this.selectDevice(
					this.options.app,
					selectedDevice.id
				);
			} else {
			...
			if (
				this.options.app.os.toLowerCase() ===
				AppCenterOS.Android.toLowerCase()
			) {
				progress.report({
					message: Messages.MakingBundleProgressMessage,
				});
			...
			progress.report({
				message: Messages.PreparingBuildProgressMessage,
			});
			...
			progress.report({
				message: Messages.UploadingAndRunningTestsProgressMessage,
			});
			const args = [
				"test",
				"run",
	...
	private async getDevicesList(
		app: CurrentApp
	): Promise<TestQuickPickItem[]> {
		let configs: models.DeviceConfiguration[] =
			await this.options.client.test.getDeviceConfigurations(
				app.ownerName,
				app.appName
			);
	...
	private async getDeviceSetsList(
		app: CurrentApp
	): Promise<TestQuickPickItem[]> {
		let configs: models.DeviceSet[] =
			await this.options.client.test.listDeviceSetsOfOwner(
				app.ownerName,
				app.appName
			);
		// Sort devices list like it was done on AppCenter Portal
		...
		return configs.map(
			(config: models.DeviceSet) =>
				<TestQuickPickItem>{
					label: `${config.name}`,
					description: MenuStrings.DeviceSetsDescription(
						config.owner.type
					),
					id: config.id,
	...
	private async selectDevice(
		...
		const deviceSelection: any =
			await this.options.client.test.createDeviceSelection(
				app.ownerName,
				app.appName,
				[deviceId]
			);
	...
	private getDefautEntryFileName(): string {
		const entryFilePath: string = path.join(
			this.options.appDirPath,
			`index.${this.getAppOsString()}.js`
		);
