 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _getReleaseUpdateDevicesStatus(
	releaseId,
	resignId,
	ownerName,
	appName,
 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _getUpdateDevicesStatus(
...
function _listCsvFormat(
	...
	const unprovisionedOnly =
		options && options.unprovisionedOnly !== undefined
			? options.unprovisionedOnly
			: false;
	const udids =
		options && options.udids !== undefined ? options.udids : undefined;
	// Validate
	try {
		if (
			distributionGroupName === null ||
		...
		if (
			unprovisionedOnly !== null &&
			unprovisionedOnly !== undefined &&
			typeof unprovisionedOnly !== "boolean"
		) {
	...
	if (unprovisionedOnly !== null && unprovisionedOnly !== undefined) {
		queryParameters.push(
			`unprovisioned_only=${encodeURIComponent(
				unprovisionedOnly.toString(),
			)}`,
		);
	}
...
function _list(distributionGroupName, ownerName, appName, options, callback) {
	...
	const releaseId =
		options && options.releaseId !== undefined
			? options.releaseId
			: undefined;
	// Validate
	try {
...
class Devices {
	...
	registerUserForDevice(userId, body, options, optionalCallback) {
		...
		if (optionalCallback) {
			return this._registerUserForDevice(
				userId,
				body,
				options,
				optionalCallback,
	...
	deviceDetails(deviceUdid, options, optionalCallback) {
		...
		if (optionalCallback) {
			return this._deviceDetails(deviceUdid, options, optionalCallback);
		} else {
			return new Promise((resolve, reject) => {
				this._deviceDetails(
					deviceUdid,
	...
	userDevicesListWithHttpOperationResponse(options) {
		const client = this.client;
		return new Promise((resolve, reject) => {
			this._userDevicesList(options, (err, result, request, response) => {
	...
	userDevicesList(options, optionalCallback) {
		...
		if (optionalCallback) {
			return this._userDevicesList(options, optionalCallback);
		} else {
			return new Promise((resolve, reject) => {
				this._userDevicesList(
					options,
					(err, result, request, response) => {
						if (err) {
							reject(err);
	 ...
	 * @reject {Error} - The error object.
	 */
	getReleaseUpdateDevicesStatusWithHttpOperationResponse(
		releaseId,
		resignId,
		ownerName,
		appName,
		...
		const client = this.client;
		return new Promise((resolve, reject) => {
			this._getReleaseUpdateDevicesStatus(
				releaseId,
				resignId,
				ownerName,
				appName,
	...
	getReleaseUpdateDevicesStatus(
		...
		if (optionalCallback) {
			return this._getReleaseUpdateDevicesStatus(
				releaseId,
				resignId,
				ownerName,
				appName,
	 ...
	 * @reject {Error} - The error object.
	 */
	getUpdateDevicesStatusWithHttpOperationResponse(
		distributionGroupName,
		resignId,
		ownerName,
		appName,
	...
	getUpdateDevicesStatus(
		...
		if (optionalCallback) {
			return this._getUpdateDevicesStatus(
				distributionGroupName,
				resignId,
				ownerName,
				appName,
