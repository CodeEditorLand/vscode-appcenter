...
function _resendInvite(
	...
	const userEmails =
		options && options.userEmails !== undefined
			? options.userEmails
			: undefined;
	// Validate
	try {
		...
		if (Array.isArray(userEmails)) {
			for (let i = 0; i < userEmails.length; i++) {
				if (
					userEmails[i] !== null &&
					userEmails[i] !== undefined &&
					typeof userEmails[i].valueOf() !== "string"
				) {
					...
					const resultMapper = {
						...
						type: {
							...
							element: {
								required: false,
								serializedName:
									"DistributionGroupUserDeleteResponseElementType",
								type: {
									name: "Composite",
									className:
...
function _listUsers(
	...
	const excludePendingInvitations =
		options && options.excludePendingInvitations !== undefined
			? options.excludePendingInvitations
			: undefined;
	// Validate
	try {
		if (
		...
		if (
			excludePendingInvitations !== null &&
			excludePendingInvitations !== undefined &&
	...
	if (
		...
		queryParameters.push(
			`exclude_pending_invitations=${encodeURIComponent(
				excludePendingInvitations.toString(),
			)}`,
		);
	}
					...
					const resultMapper = {
						...
						type: {
							...
							element: {
								required: false,
								serializedName:
									"DistributionGroupUserPostResponseElementType",
								type: {
									name: "Composite",
									className:
...
function _update(ownerName, appName, distributionGroupName, options, callback) {
	...
	const isPublic =
		options && options.isPublic !== undefined
			? options.isPublic
			: undefined;
	// Validate
	try {
	...
	if (
		...
		distributionGroup = new client.models[
...
class DistributionGroups {
	 ...
	 */
	resendInviteWithHttpOperationResponse(
		ownerName,
		appName,
		distributionGroupName,
		options,
	 ...
	 * @reject {Error} - The error object.
	 */
	listUsersWithHttpOperationResponse(
		ownerName,
		appName,
		distributionGroupName,
		options,
