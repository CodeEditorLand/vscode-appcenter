...
function _get(crashGroupId, crashId, ownerName, appName, options, callback) {
	...
	if (includeStacktrace !== null && includeStacktrace !== undefined) {
		queryParameters.push(
			`include_stacktrace=${encodeURIComponent(
				includeStacktrace.toString(),
			)}`,
		);
	}
	if (groupingOnly !== null && groupingOnly !== undefined) {
	...
	return client.pipeline(httpRequest, (err, response, responseBody) => {
		...
		if (statusCode === 201) {
			...
			try {
				...
				result = JSON.parse(responseBody);
				if (parsedResponse !== null && parsedResponse !== undefined) {
					const resultMapper = new client.models["Crash"]().mapper();
					result = client.deserialize(
						resultMapper,
						parsedResponse,
						"result",
...
class Crashes {
	...
	getHockeyAppCrashForwardingStatus(
		...
		if (optionalCallback) {
			return this._getHockeyAppCrashForwardingStatus(
				ownerName,
				appName,
				options,
				optionalCallback,
	...
	updateHockeyAppCrashForwardingWithHttpOperationResponse(
		...
		const client = this.client;
		return new Promise((resolve, reject) => {
			this._updateHockeyAppCrashForwarding(
				ownerName,
				appName,
				options,
				(err, result, request, response) => {
	...
	updateHockeyAppCrashForwarding(
		...
		if (optionalCallback) {
			return this._updateHockeyAppCrashForwarding(
				ownerName,
				appName,
				options,
				optionalCallback,
	...
	getAppCrashesInfo(ownerName, appName, options, optionalCallback) {
		...
		if (optionalCallback) {
			return this._getAppCrashesInfo(
				ownerName,
				appName,
				options,
				optionalCallback,
	...
	getCrashTextAttachmentContentWithHttpOperationResponse(
		...
		const client = this.client;
		return new Promise((resolve, reject) => {
			this._getCrashTextAttachmentContent(
				crashId,
				attachmentId,
				ownerName,
				appName,
	...
	getCrashTextAttachmentContent(
		...
		if (optionalCallback) {
			return this._getCrashTextAttachmentContent(
				crashId,
				attachmentId,
				ownerName,
				appName,
	...
	getCrashAttachmentLocationWithHttpOperationResponse(
		...
		const client = this.client;
		return new Promise((resolve, reject) => {
			this._getCrashAttachmentLocation(
				crashId,
				attachmentId,
				ownerName,
				appName,
	...
	getCrashAttachmentLocation(
		...
		if (optionalCallback) {
			return this._getCrashAttachmentLocation(
				crashId,
				attachmentId,
				ownerName,
				appName,
	...
	getRawCrashLocation(
		...
		if (optionalCallback) {
			return this._getRawCrashLocation(
				crashGroupId,
				crashId,
				ownerName,
				appName,
	...
	getNativeCrashDownload(
		...
		if (optionalCallback) {
			return this._getNativeCrashDownload(
				crashGroupId,
				crashId,
				ownerName,
				appName,
