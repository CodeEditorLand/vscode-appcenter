...
describe("Extension Tests", function () {
	describe("#registerAppCenterCommands", function () {
		it("should register all commands", async function () {
			...
			const appCenterCommands = new Set(
				registeredCommands.filter((c) =>
					c.startsWith(CommandNames.CommandPrefix)
				)
			);

			for (const requiredCommand of requiredCommands) {
				should.equal(
					appCenterCommands.has(requiredCommand),
					true,
					`Command not registered: ${requiredCommand}`
				);
			}

			for (const registeredCommand of appCenterCommands) {
				should.equal(
					requiredCommands.has(registeredCommand),
					true,
					`Unexpected command registered: ${registeredCommand}`
				);
			}
		});
	});
