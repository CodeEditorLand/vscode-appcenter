...
describe("Auth", function () {
	...
	before(async () => {
		...
		stubPrototype =
			os.platform() === "win32"
				? WinTokenStore.prototype
				: os.platform() === "darwin"
				? OsxTokenStore.prototype
				: FileTokenStore.prototype;
	...
	describe("#doLogin", () => {
		...
		before(() => {
			...
			setTokenStub
				.withArgs(getUserInfoStub.userId, { token: mockToken })
				.resolves();
	...
	describe("#doLogout", () => {
		...
		before(() => {
			...
			sandbox.stub(vstsProfileStorage, "activeProfile").get(() => {
				return null;
			});
			removeTokenStub = sandbox.stub(stubPrototype, "remove");
			removeTokenStub.withArgs(mockId).resolves();
