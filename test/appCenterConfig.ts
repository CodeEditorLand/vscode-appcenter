...
describe("AppCenterConfig", function () {
	...
	before(() => {
		...
		appCenterConfig = Utils.createAppCenterConfigFrom(
			appName,
			"",
			loggerStub
		);
	...
	describe("#setAndroidStringResourcesDeploymentKey", () => {
		it("should set the deployment key", async () => {
			...
			const data = fs.readFileSync(androidStringsPath, {
				encoding: "utf8",
			});
	...
	describe("#configPlistValue", () => {
		it("should set and get config plist value", async () => {
			...
			const data = fs.readFileSync(pathToAppCenterConfigPlist, {
				encoding: "utf8",
			});
	...
	describe("#androidconfigValue", () => {
		it("should set and get android config value", async () => {
			...
			const newValue =
				appCenterConfig.getAndroidAppCenterConfigValueByKey(key);
			newValue.should.equal(value);
			const data = fs.readFileSync(pathToAndroidConfig, {
				encoding: "utf8",
			});
